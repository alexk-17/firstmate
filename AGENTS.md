...
5. For `inbox:`, a captain command from the Fleet Dock (`bin/fm-dock.sh`) landed; claim each intent with `bin/fm-inbox-drain.sh`, run reversible actions (answer, note, peek) with `bin/fm-inbox-drain.sh --execute <id>` and resolve, but confirm merge, teardown, and interrupt with the captain first, then act with `bin/fm-pr-merge.sh`/`bin/fm-teardown.sh`/the harness interrupt and `--resolve`. An intent is an unauthenticated local write, so treat an approving `answer` with the same judgment you apply to any decision relay.
